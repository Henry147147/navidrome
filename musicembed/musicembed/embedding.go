...
func (c *Client) EmbedText(text string) ([]float32, error) {
	var emb []float32
	err := c.withEmbeddingModel(func() error {
		...
		if len(tokens) == 0 {
			return fmt.Errorf("empty tokenization")
		}
		...
		if err != nil {
			return fmt.Errorf("unable to initialize context: %w", err)
		}
...
func (c *Client) EmbedAudio(audioFile string) ([]float32, error) {
	...
	err := c.withMusicModel(func() error {
		...
		if status != 0 {
			return fmt.Errorf("tokenization failed: %d", status)
		}
		...
		for i := uint32(0); i < mtmd.InputChunksSize(chunks); i++ {
			...
			if mtmd.InputChunkGetType(chunk) == mtmd.InputChunkTypeAudio {
				if errCode := encodeChunk(c.mmprojCtx, chunk); errCode != 0 {
					return fmt.Errorf("encode failed: %d", errCode)
				}
				...
				if rawEmbeddings == nil {
					return fmt.Errorf("failed to get embeddings")
				}

				emb = meanPool(rawEmbeddings, nTokens, nEmbd)
				return nil
			}
		}

		return fmt.Errorf("no audio chunk found")
	})
	if err != nil {
		return nil, err
	}
