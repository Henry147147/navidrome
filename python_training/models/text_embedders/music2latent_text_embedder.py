"""Text embedder for Music2Latent audio embeddings (576D)."""
...
class Music2LatentTextEmbedder(BaseTextEmbedder):
    """
    Text embedding model that produces 576D embeddings to match Music2Latent audio embeddings.

    Architecture:
    - Base: DistilBERT or small BERT variant
    - Hidden dim: 768
    - Projection: 768 -> 192
    - Enrichment: 192 -> 576 (mean, robust_sigma, dmean)
    """

    def __init__(
        self,
        base_model: str = "distilbert-base-uncased",
        target_dim: int = 192,  # Before enrichment
        enrichment_enabled: bool = True,
        dropout: float = 0.1,
        logger: Optional[logging.Logger] = None,
    ):
        """
        Initialize Music2Latent text embedder.

        Args:
            base_model: HuggingFace model name for the base transformer
            target_dim: Target dimension before enrichment (192)
        ...
        self.logger.info(f"Initialized Music2Latent text embedder: {self.hidden_dim} -> {target_dim} -> {self.output_dim}")
    ...
    def save_pretrained(self, save_directory: str) -> None:
        ...
        import json
        config = {
            'model_type': 'Music2LatentTextEmbedder',
...
if __name__ == "__main__":
    ...
    model = Music2LatentTextEmbedder()
    ...
    print(f"Embeddings shape: {embeddings.shape}")  # Should be [3, 576]
